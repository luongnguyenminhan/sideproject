...
def create_websocket_token(user_data: dict, expires_delta_minutes: int = 60) -> str:
    ...
    try:
        print(
            f"\033[94m[create_websocket_token] Creating WebSocket token for user: {user_data.get('user_id')}\033[0m"
        )
        ...
        print(
            f"\033[92m[create_websocket_token] WebSocket token created successfully\033[0m"
        )
        return token

    except Exception as e:
        print(
            f"\033[91m[create_websocket_token] ERROR: Failed to create WebSocket token - {e}\033[0m"
        )
        raise CustomHTTPException(
