...
logger.info(
    f"MinIO config: endpoint={settings.MINIO_ENDPOINT}, access_key={settings.MINIO_ACCESS_KEY}, bucket_name={settings.MINIO_BUCKET_NAME}, secure={secure_value}"
)
...
class MinioHandler:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Initializing MinIO client with endpoint: {settings.MINIO_ENDPOINT}"
        )
    ...
    def _generate_safe_object_name(
        self, meeting_id: str, file_name: str, file_type: str = "audio"
    ) -> str:
        """
    ...
    async def upload_file(
        ...
        try:
            logger.info(
                f"Starting upload of '{file_name}', size: {len(file_content)} bytes"
            )
            ...
            object_name = self._generate_safe_object_name(
                meeting_id, file_name, file_type
            )
            ...
            logger.info(
                f"File '{file_name}' uploaded successfully to MinIO as '{object_name}'"
            )
            return object_name
    ...
    async def upload_fastapi_file(
        self, file: UploadFile, meeting_id: str, file_type: str = "audio"
    ) -> str:
        """
    ...
    async def upload_bytes(
        ...
        try:
            logger.info(
                f"Starting upload of '{filename}' as bytes, size: {len(content)} bytes"
            )
