...
class N8NAPIClient:
	...
	def __init__(self):
		...
		self.chat_webhook_endpoint = '/webhook/786eb3d9-73e7-406e-acd9-3e4dfcb67e87/chat'
		self.timeout = 30.0
	...
	async def call_chat_workflow(
		...
		"""
		logger.info(f'🚀 [N8NAPIClient] Calling N8N chat workflow for conversation: {conversation_id}')
		...
		try:
			...
			logger.info(f'📥 [N8NAPIClient] Chat workflow response status: {response.status_code}')
				...
				if isinstance(response_data, list) and len(response_data) > 0:
					...
					return {
						...
						'usage': {
							...
							'total_tokens': len(user_message.split()) + len(output_content.split()),
						},
						'response_time_ms': response_time_ms,
					}
				else:
					logger.error(f'❌ [N8NAPIClient] Unexpected response format: {response_data}')
					raise HTTPException(
						status_code=500,
						detail='N8N Chat workflow returned unexpected format',
					)
			else:
				logger.error(f'❌ [N8NAPIClient] Chat workflow failed with status: {response.status_code}')
			...
			logger.error(f'⏰ [N8NAPIClient] Chat workflow timeout after {self.timeout}s')
			raise HTTPException(status_code=408, detail='N8N Chat workflow API request timeout')
		except httpx.RequestError as e:
			logger.error(f'🌐 [N8NAPIClient] Chat workflow network error: {str(e)}')
			raise HTTPException(status_code=503, detail=f'N8N Chat workflow API network error: {str(e)}')
