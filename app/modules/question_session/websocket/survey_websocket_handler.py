...
class SurveyWebSocketHandler:
	...
	async def handle_survey_response_message(self, websocket_connection, message: Dict[str, Any], user_id: str) -> None:
		"""
		...
		try:
			...
			request = ParseSurveyResponseRequest(type='survey_response', answers=answers, conversation_id=conversation_id, timestamp=message.get('timestamp'))

			# Process survey response with AI integration
			result = await self.integration_service.process_survey_and_chat_response(survey_request=request.model_dump(), conversation_id=conversation_id, user_id=user_id)
			...
			if result.get('ai_response', {}).get('content'):
				await self._send_ai_chat_message(websocket_connection, result['ai_response']['content'], conversation_id)
	...
	async def handle_format_survey_message(self, websocket_connection, message: Dict[str, Any], user_id: str) -> None:
		"""
		...
		try:
			# Create request object
			request = ParseSurveyResponseRequest(type='survey_response', answers=message.get('answers', {}), conversation_id=message.get('conversation_id'), timestamp=message.get('timestamp'))

			# Format as human message
			human_message = await self.survey_processor.format_survey_response_as_human_message(request=request, user_id=user_id)

			# Send formatted message back
			response = {'type': 'survey_formatted', 'conversation_id': message.get('conversation_id'), 'human_message': human_message, 'timestamp': message.get('timestamp'), 'success': True}
	...
	async def _send_acknowledgment(self, websocket_connection, conversation_id: str) -> None:
		"""Send processing acknowledgment"""
		ack_message = {'type': 'survey_processing_started', 'conversation_id': conversation_id, 'message': 'Survey response received and processing started', 'timestamp': self._get_current_timestamp()}
		await websocket_connection.send_json(ack_message)

	async def _send_survey_result(self, websocket_connection, result: Dict[str, Any]) -> None:
		"""Send comprehensive survey processing result"""
		response_message = {'type': 'survey_processing_complete', 'data': result, 'timestamp': self._get_current_timestamp()}
		await websocket_connection.send_json(response_message)

	async def _send_ai_chat_message(self, websocket_connection, ai_content: str, conversation_id: str) -> None:
		"""Send AI response as a chat message"""
		chat_message = {'type': 'ai_message', 'conversation_id': conversation_id, 'content': ai_content, 'source': 'survey_analysis', 'timestamp': self._get_current_timestamp()}
		await websocket_connection.send_json(chat_message)

	async def _send_error(self, websocket_connection, error_message: str) -> None:
		"""Send error message"""
		error_response = {'type': 'survey_error', 'error': error_message, 'timestamp': self._get_current_timestamp()}
...
class SurveyWebSocketMessages:
	...
	def survey_response_message(conversation_id: str, answers: Dict[str, Any]) -> Dict[str, Any]:
		...
		return {'type': 'survey_response', 'conversation_id': conversation_id, 'answers': answers, 'timestamp': SurveyWebSocketHandler()._get_current_timestamp()}

	@staticmethod
	def format_survey_message(conversation_id: str, answers: Dict[str, Any]) -> Dict[str, Any]:
		"""Create a format survey message"""
		return {'type': 'format_survey', 'conversation_id': conversation_id, 'answers': answers, 'timestamp': SurveyWebSocketHandler()._get_current_timestamp()}

	@staticmethod
	def survey_processing_complete_response(result: Dict[str, Any]) -> Dict[str, Any]:
		"""Create a processing complete response"""
		return {'type': 'survey_processing_complete', 'data': result, 'timestamp': SurveyWebSocketHandler()._get_current_timestamp()}
