...
class QuestionAnswerDAL(BaseDAL[QuestionAnswer]):
	...
	def get_answer_by_question(self, session_id: str, question_id: str) -> Optional[QuestionAnswer]:
		...
		return self.db.query(self.model).filter(and_(self.model.session_id == session_id, self.model.question_id == question_id, self.model.is_deleted == False)).first()

	def create_or_update_answer(self, session_id: str, question_id: str, question_text: Optional[str], answer_data: dict, answer_type: str) -> QuestionAnswer:
		...
		if existing_answer:
			...
			update_data = {'answer_data': answer_data, 'answer_type': answer_type, 'submitted_at': datetime.utcnow()}
			if question_text:
			...
			# Create new answer
			answer_data_dict = {'session_id': session_id, 'question_id': question_id, 'question_text': question_text, 'answer_data': answer_data, 'answer_type': answer_type, 'submitted_at': datetime.utcnow()}
	...
	def get_answers_by_question_ids(self, session_id: str, question_ids: List[str]) -> List[QuestionAnswer]:
		...
		return self.db.query(self.model).filter(and_(self.model.session_id == session_id, self.model.question_id.in_(question_ids), self.model.is_deleted == False)).all()
