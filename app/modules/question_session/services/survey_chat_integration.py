...
class SurveyChatIntegrationService:
	...
	async def process_survey_and_chat_response(self, survey_request: Dict[str, Any], conversation_id: str, user_id: str, custom_ai_prompt: Optional[str] = None) -> Dict[str, Any]:
		"""
		...
		try:
			...
			human_message = await self.survey_processor.format_survey_response_as_human_message(request=survey_request_obj, user_id=user_id)
			...
			ai_result = await self.langgraph_service.execute_conversation(agent=agent, conversation_id=conversation_id, user_message=ai_prompt, conversation_system_prompt=self._get_survey_analysis_system_prompt(), conversation_history=conversation_history, user_id=user_id)

			# Step 6: Format comprehensive response
			result = {
				'survey_data': {'conversation_id': conversation_id, 'user_id': user_id, 'survey_answers': survey_request.get('answers', {}), 'processed_at': datetime.now().isoformat()},
				'human_message': human_message,
				'ai_response': {'content': ai_result.get('content', ''), 'metadata': ai_result.get('metadata', {}), 'processing_time': ai_result.get('metadata', {}).get('response_time_ms', 0)},
			...
			return {'survey_data': survey_request, 'human_message': None, 'ai_response': {'content': f'I apologize, but I encountered an error while processing your survey responses: {str(e)}', 'metadata': {}, 'processing_time': 0}, 'integration_metadata': {'success': False, 'error': str(e), 'agent_used': None}}
...
"""
	...
	async def send_survey_response_to_chat(self, survey_response_text: str, conversation_id: str, user_id: str, websocket_connection=None) -> Dict[str, Any]:
		"""
		...
		try:
			...
			if websocket_connection and hasattr(websocket_connection, 'send_json'):
				message_data = {'type': 'human_message', 'content': survey_response_text, 'conversation_id': conversation_id, 'user_id': user_id, 'source': 'survey_response', 'timestamp': datetime.now().isoformat()}
			...
			return {'success': True, 'conversation_id': conversation_id, 'message_sent': True, 'message_length': len(survey_response_text), 'sent_via_websocket': websocket_connection is not None, 'timestamp': datetime.now().isoformat()}

		except Exception as e:
			logger.error(f'Error sending survey response to chat: {e}')
			return {'success': False, 'error': str(e), 'conversation_id': conversation_id, 'message_sent': False}
