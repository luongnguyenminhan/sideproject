...
class QuestionSessionDAL(BaseDAL[QuestionSession]):
    ...
    def get_active_sessions(self) -> List[QuestionSession]:
        ...
        return (
            self.db.query(self.model)
            .filter(self.model.status == "active", self.model.is_deleted == False)
            .all()
        )
    ...
    def get_by_status(self, status: str) -> List[QuestionSession]:
        ...
        return (
            self.db.query(self.model)
            .filter(self.model.status == status, self.model.is_deleted == False)
            .all()
        )
    ...
    def save_generated_questions(
        self, session_id: str, generated_questions: list, all_previous_questions: list
    ) -> Optional[QuestionSession]:
