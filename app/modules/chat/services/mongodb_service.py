...
class MongoDBService:
	...
	def __init__(self):
		...
		print(f'\033[94m[MongoDBService.__init__] Database name: {database_name}\033[0m')
		...
		try:
			...
			print(f'\033[92m[MongoDBService.__init__] MongoDB ping successful: {result}\033[0m')

			# Test database access
			collections = self.db.list_collection_names()
			print(f'\033[92m[MongoDBService.__init__] Available collections: {collections}\033[0m')

		except Exception as e:
			print(f'\033[91m[MongoDBService.__init__] ERROR: Failed to connect to MongoDB - {e}\033[0m')
			raise e

	def save_message(self, message_data: Dict[str, Any]) -> str:
		"""Save a chat message to MongoDB"""
		try:
			print(f'\033[94m[MongoDBService.save_message] Attempting to save message: {message_data}\033[0m')
			result = self.messages_collection.insert_one(message_data)
			print(f'\033[92m[MongoDBService.save_message] Message saved successfully with ID: {result.inserted_id}\033[0m')
			logger.info(f'Message saved to MongoDB with ID: {result.inserted_id}')
			return str(result.inserted_id)
		except Exception as e:
			print(f'\033[91m[MongoDBService.save_message] ERROR saving message: {e}\033[0m')
			logger.error(f'Error saving message to MongoDB: {e}')
			raise

	def get_conversation_messages(self, conversation_id: str, limit: int = 50, skip: int = 0) -> List[Dict[str, Any]]:
		"""Get messages for a conversation from MongoDB"""
		try:
			messages = self.messages_collection.find({'conversation_id': conversation_id}).sort('timestamp', DESCENDING).skip(skip).limit(limit)
	...
	def get_conversation_history(self, conversation_id: str, limit: int = 10) -> List[Dict[str, Any]]:
		...
		try:
			messages = self.messages_collection.find({'conversation_id': conversation_id}).sort('timestamp', DESCENDING).limit(limit)
	...
	def delete_conversation_messages(self, conversation_id: str):
		...
		try:
			result = self.messages_collection.delete_many({'conversation_id': conversation_id})
			logger.info(f'Deleted {result.deleted_count} messages for conversation: {conversation_id}')

			# Also delete conversation summary
			self.conversations_collection.delete_one({'conversation_id': conversation_id})
		except Exception as e:
