...
class MessageDAL(BaseDAL[Message]):
    ...
    def soft_delete_by_conversation(self, conversation_id: str):
        ...
        logger.info(
            f"\033[93m[MessageDAL.soft_delete_by_conversation] Soft deleting all messages for conversation: {conversation_id}\033[0m"
        )
        updated_count = (
            self.db.query(self.model)
            .filter(
        ...
        logger.info(
            f"\033[92m[MessageDAL.soft_delete_by_conversation] Soft deleted {updated_count} messages for conversation: {conversation_id}\033[0m"
        )
