...
class MessageDAL(BaseDAL[Message]):
	def __init__(self, db: Session):
		print(f'\033[96m[MessageDAL.__init__] Initializing MessageDAL with db session: {db}\033[0m')
		super().__init__(db, Message)
		print(f'\033[92m[MessageDAL.__init__] MessageDAL initialized successfully\033[0m')
	...
	def get_conversation_messages(
		...
		print(f'\033[94m[MessageDAL.get_conversation_messages] Base query created for conversation messages\033[0m')

		# If before_message_id is provided, get messages before that message
		if before_message_id:
			print(f'\033[94m[MessageDAL.get_conversation_messages] Applying before_message filter: {before_message_id}\033[0m')
			before_message = self.get_by_id(before_message_id)
			if before_message:
				print(f'\033[94m[MessageDAL.get_conversation_messages] Found before_message, filtering by timestamp: {before_message.timestamp}\033[0m')
				query = query.filter(self.model.timestamp < before_message.timestamp)
			else:
				print(f'\033[95m[MessageDAL.get_conversation_messages] Before message not found: {before_message_id}\033[0m')
		...
		print(f'\033[92m[MessageDAL.get_conversation_messages] Pagination completed, returning results\033[0m')
		return paginated_result

	def get_conversation_history(self, conversation_id: str, limit: int = 10) -> List[Message]:
		"""Get recent messages for conversation context"""
		print(f'\033[93m[MessageDAL.get_conversation_history] Getting conversation history for: {conversation_id}, limit: {limit}\033[0m')
		messages = (
		...
		print(f'\033[92m[MessageDAL.get_conversation_history] Found {len(messages)} messages in history\033[0m')
		return messages

	def get_latest_message(self, conversation_id: str) -> Optional[Message]:
		"""Get the latest message in a conversation"""
		print(f'\033[93m[MessageDAL.get_latest_message] Getting latest message for conversation: {conversation_id}\033[0m')
		message = (
			self.db.query(self.model)
			.filter(
				self.model.conversation_id == conversation_id,
		...
		if message:
			print(f'\033[92m[MessageDAL.get_latest_message] Found latest message: {message.id}, role: {message.role}, timestamp: {message.timestamp}\033[0m')
		else:
