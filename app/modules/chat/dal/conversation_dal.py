...
class ConversationDAL(BaseDAL[Conversation]):
    def __init__(self, db: Session):
        logger.info(
            f"\033[96m[ConversationDAL.__init__] Initializing ConversationDAL with db session: {db}\033[0m"
        )
        super().__init__(db, Conversation)
        logger.info(
            f"\033[92m[ConversationDAL.__init__] ConversationDAL initialized successfully\033[0m"
        )
    ...
    def get_user_conversations(
        ...
        logger.info(
            f"\033[94m[ConversationDAL.get_user_conversations] Base query created for user conversations\033[0m"
        )

        # Apply search filter
        if search:
            logger.info(
                f"\033[94m[ConversationDAL.get_user_conversations] Applying search filter: {search}\033[0m"
            )
            query = query.filter(self.model.name.ilike(f"%{search}%"))

        # Apply ordering
        logger.info(
            f"\033[94m[ConversationDAL.get_user_conversations] Applying ordering by: {order_by} {order_direction}\033[0m"
        )
        ...
        logger.info(
            f"Found {total_count} conversations, returning page {page} with {len(conversations)} items"
        )

        paginated_result = Pagination(
            items=conversations, total_count=total_count, page=page, page_size=page_size
        )
        logger.info(
    ...
    def get_user_conversation_by_id(
        ...
        logger.info(
            f"\033[93m[ConversationDAL.get_user_conversation_by_id] Getting conversation: {conversation_id} for user: {user_id}\033[0m"
        )
        conversation = (
        ...
        if conversation:
            ...
            logger.info(
                f"\033[95m[ConversationDAL.get_user_conversation_by_id] Conversation not found: {conversation_id}\033[0m"
            )
        return conversation
