...
class ChatRepo:
    def __init__(self, db: Session = Depends(get_db)):
        ...
        logger.debug(
            f"\033[92m[ChatRepo.__init__] ChatRepo initialized successfully with Agent Integration\033[0m"
        )
    ...
    def create_message(
        ...
        with self.message_dal.transaction():
            logger.debug(
                f"\033[94m[ChatRepo.create_message] Creating message in MySQL\033[0m"
            )
    ...
    async def get_ai_response(
        ...
        if not user_id:
            ...
            logger.debug(
                f"\033[94m[ChatRepo.get_ai_response] Retrieved user_id from conversation: {user_id}\033[0m"
            )
        ...
        if not api_key:
            ...
            if default_key:
                ...
                logger.debug(
                    f"\033[92m[ChatRepo.get_ai_response] Found default API key for GOOGLE\033[0m"
                )

        try:
            logger.debug(
                f"\033[94m[ChatRepo.get_ai_response] Calling Agent Integration Service\033[0m"
            )
            logger.debug(
                f"\033[96m[ChatRepo.get_ai_response] Using API key: {api_key}... (truncated for security)\033[0m"
            )
            ...
            logger.debug(
                f'\033[92m[ChatRepo.get_ai_response] Agent response received, agent: {result.get("agent_name", "unknown")}, response_time: {result.get("response_time_ms", 0)}ms\033[0m'
            )
            return result

        except Exception as e:
            logger.debug(
                f"\033[91m[ChatRepo.get_ai_response] Error getting Agent response: {e}\033[0m"
            )
            logger.error(f"Error getting Agent response: {e}")

            # Fallback to simulation if agent system fails
            logger.debug(
                f"\033[93m[ChatRepo.get_ai_response] Falling back to simulation\033[0m"
            )
    ...
    async def get_ai_response_streaming(
        ...
        if not user_id:
            ...
            user_id = conversation.user_id
            logger.debug(
                f"\033[94m[ChatRepo.get_ai_response_streaming] Retrieved user_id from conversation: {user_id}\033[0m"
            )
        ...
        if not api_key:
            ...
            if default_key:
                api_key = default_key.get_api_key()
                logger.debug(
                    f"\033[92m[ChatRepo.get_ai_response_streaming] Found default API key for Google\033[0m"
                )

        try:
            logger.debug(
                f"\033[94m[ChatRepo.get_ai_response_streaming] Calling Agent Integration Service for streaming\033[0m"
            )
            ...
            logger.debug(
                f"\033[91m[ChatRepo.get_ai_response_streaming] Error getting Agent streaming response: {e}\033[0m"
            )
            logger.error(f"Error getting Agent streaming response: {e}")

            # Fallback to simulation if agent system fails
            logger.debug(
                f"\033[93m[ChatRepo.get_ai_response_streaming] Falling back to streaming simulation\033[0m"
            )
    ...
    async def _simulate_ai_response_fallback(self, message: str, api_key: str) -> dict:
        ...
        """
        logger.debug(
            f"\033[93m[ChatRepo._simulate_ai_response_fallback] Running fallback simulation for message_length: {len(message)}\033[0m"
        )
        ...
        if "hello" in message.lower():
            ...
            response_content += (
                "Please try again in a moment when the full AI system is restored."
            )
    ...
    async def _simulate_streaming_ai_response_fallback(
        ...
        """
        logger.debug(
            f"\033[93m[ChatRepo._simulate_streaming_ai_response_fallback] Running fallback streaming simulation for user: {user_id}\033[0m"
        )
        start_time = time.time()
        ...
        if websocket_manager and user_id:
            logger.debug(
                f"\033[94m[ChatRepo._simulate_streaming_ai_response_fallback] Starting fallback streaming\033[0m"
            )
