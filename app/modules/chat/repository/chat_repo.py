...
class ChatRepo:
	...
	def create_message(
		...
		with self.message_dal.transaction():
			...
			print(f'\033[96m[ChatRepo.create_message] Created mongo_message_data for MongoDB\033[0m')

			try:
				print(f'\033[94m[ChatRepo.create_message] Saving message to MongoDB\033[0m')
				mongodb_service.save_message(mongo_message_data)
				print(f'\033[92m[ChatRepo.create_message] Message saved to MongoDB successfully\033[0m')
			except Exception as e:
				print(f'\033[91m[ChatRepo.create_message] Failed to save message to MongoDB: {e}\033[0m')
	...
	async def get_ai_response(self, conversation_id: str, user_message: str, api_key: str = None) -> dict:
		"""Get AI response for a message (non-streaming)"""
		print(f'\033[93m[ChatRepo.get_ai_response] Getting AI response for conversation: {conversation_id}, message_length: {len(user_message)}, has_api_key: {api_key is not None}\033[0m')
		start_time = time.time()

		# Get conversation history for context from MongoDB
		try:
			print(f'\033[94m[ChatRepo.get_ai_response] Getting conversation history from MongoDB\033[0m')
			conversation_history = mongodb_service.get_conversation_history(conversation_id=conversation_id, limit=10)
			print(f'\033[92m[ChatRepo.get_ai_response] Retrieved {len(conversation_history)} messages from history\033[0m')
		except Exception as e:
			print(f'\033[91m[ChatRepo.get_ai_response] Failed to get conversation history: {e}\033[0m')
		...
		if not api_key:
			...
			print(f'\033[94m[ChatRepo.get_ai_response] Getting default API key for user: {conversation.user_id}\033[0m')
			default_key = self.api_key_dal.get_user_default_api_key(user_id=conversation.user_id, provider='openai')
			if default_key:
		...
		try:
			print(f'\033[94m[ChatRepo.get_ai_response] Calling AI response simulation\033[0m')
			ai_response = await self._simulate_ai_response(message=user_message, history=conversation_history, api_key=api_key)
			response_time = int((time.time() - start_time) * 1000)
			print(f'\033[92m[ChatRepo.get_ai_response] AI response received, response_time: {response_time}ms, content_length: {len(ai_response["content"])}\033[0m')
			...
			print(f'\033[91m[ChatRepo.get_ai_response] Error getting AI response: {e}\033[0m')
	...
	async def get_ai_response_streaming(
		...
		# Get conversation history for context from MongoDB
		try:
			print(f'\033[94m[ChatRepo.get_ai_response_streaming] Getting conversation history from MongoDB\033[0m')
			conversation_history = mongodb_service.get_conversation_history(conversation_id=conversation_id, limit=10)
			print(f'\033[92m[ChatRepo.get_ai_response_streaming] Retrieved {len(conversation_history)} messages from history\033[0m')
		except Exception as e:
			print(f'\033[91m[ChatRepo.get_ai_response_streaming] Failed to get conversation history: {e}\033[0m')
			logger.error(f'Failed to get conversation history from MongoDB: {e}')
			conversation_history = []
		...
		if not api_key:
			...
			conversation = self.get_conversation_by_id(conversation_id, user_id)
			print(f'\033[94m[ChatRepo.get_ai_response_streaming] Getting default API key for user: {conversation.user_id}\033[0m')
			default_key = self.api_key_dal.get_user_default_api_key(user_id=conversation.user_id, provider='openai')
			if default_key:
				api_key = default_key.get_api_key()
		...
		if not api_key:
			print(f'\033[91m[ChatRepo.get_ai_response_streaming] No API key available\033[0m')
			raise ValidationException(_('api_key_required'))

		try:
			print(f'\033[94m[ChatRepo.get_ai_response_streaming] Calling streaming AI response simulation\033[0m')
			...
			response_time = int((time.time() - start_time) * 1000)
			print(f'\033[92m[ChatRepo.get_ai_response_streaming] Streaming AI response completed, response_time: {response_time}ms, content_length: {len(full_response["content"])}\033[0m')
			...
			print(f'\033[91m[ChatRepo.get_ai_response_streaming] Error getting streaming AI response: {e}\033[0m')
	...
	async def _simulate_ai_response(self, message: str, history: list, api_key: str) -> dict:
		...
		"""
		print(f'\033[93m[ChatRepo._simulate_ai_response] Simulating AI response for message_length: {len(message)}, history_count: {len(history)}\033[0m')
		# Simulate processing delay
		print(f'\033[94m[ChatRepo._simulate_ai_response] Simulating processing delay\033[0m')
		...
		if 'hello' in message.lower():
			...
			print(f'\033[95m[ChatRepo._simulate_ai_response] Detected greeting in message\033[0m')
		elif 'how are you' in message.lower():
			response_content += "I'm doing well, thank you for asking!"
			print(f'\033[95m[ChatRepo._simulate_ai_response] Detected how are you question\033[0m')
		else:
			response_content += "I'm here to help you with any questions you might have."
			print(f'\033[95m[ChatRepo._simulate_ai_response] Using default response\033[0m')
		...
		print(f'\033[92m[ChatRepo._simulate_ai_response] AI response simulation completed, tokens: {response_data["usage"]["total_tokens"]}\033[0m')
	...
	async def _simulate_streaming_ai_response(
		...
		"""
		print(f'\033[93m[ChatRepo._simulate_streaming_ai_response] Simulating streaming AI response for message_length: {len(message)}, history_count: {len(history)}, user: {user_id}\033[0m')
		# Get the full response first
		full_response = await self._simulate_ai_response(message, history, api_key)
		response_text = full_response['content']
		print(f'\033[94m[ChatRepo._simulate_streaming_ai_response] Full response generated, preparing to stream\033[0m')

		if websocket_manager and user_id:
			print(f'\033[94m[ChatRepo._simulate_streaming_ai_response] Starting streaming simulation\033[0m')
			# Simulate streaming by sending chunks
			words = response_text.split()
			current_chunk = ''
			print(f'\033[96m[ChatRepo._simulate_streaming_ai_response] Will stream {len(words)} words in chunks\033[0m')
			...
			for i, word in enumerate(words):
				...
				if i % 3 == 0 or i == len(words) - 1:
					print(f'\033[95m[ChatRepo._simulate_streaming_ai_response] Sending chunk {i // 3 + 1}, words: {i + 1}/{len(words)}, is_final: {i == len(words) - 1}\033[0m')
			...
			print(f'\033[92m[ChatRepo._simulate_streaming_ai_response] Streaming simulation completed\033[0m')
