	...
	async def upload_files(
		...
		for i, file in enumerate(files):
			...
			try:
				...
				pass  # logger.info(f'\033[94m[FileRepo.upload_files] Checking for duplicates with checksum: {checksum}\033[0m')
				existing_files = self.file_dal.get_files_by_checksum(checksum, user_id)
				if existing_files:
					pass  # logger.info(f'\033[95m[FileRepo.upload_files] Duplicate found, returning existing file: {existing_files[0].id}\033[0m')
