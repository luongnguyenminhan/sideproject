...
class GlobalKBService:
	...
	def __init__(self, db: Session):
		...
		logger.info(f'[GlobalKBService] Using collection: {self.GLOBAL_COLLECTION_NAME}')
	...
	async def index_admin_documents(self, documents_data: List[Dict[str, Any]]) -> Dict[str, Any]:
		...
		if documents:
			logger.info(f'[GlobalKBService] Indexing {len(documents)} admin documents to Global KB')
...
```
	...
	async def add_general_knowledge(self, knowledge_items: List[Dict[str, Any]]) -> Dict[str, Any]:
		...
		Returns:
		    Indexing results
		"""
		logger.info(f'[GlobalKBService] Adding {len(knowledge_items)} general knowledge items')
	...
	def get_global_kb_stats(self) -> Dict[str, Any]:
		...
		try:
			...
			stats = {
				...
				'status': ('active' if self._check_collection_exists() else 'not_initialized'),
			}
