...
class LangChainQdrantService:
	...
	async def index_documents(self, documents: List[Document], collection_name: str, batch_size: int = 50) -> Dict[str, Any]:
		...
		try:
			...
			from app.modules.agentic_rag.schemas.kb_schema import AddDocumentsRequest, DocumentModel
			...
			logger.error(f'LangChainQdrantService - Failed to index documents: {str(e)}', exc_info=True)
			...
			for result in query_response.results:
				if result.score >= score_threshold:
					doc = Document(
						...
						metadata={**result.metadata, 'similarity_score': result.score, 'document_id': result.id},
					)
	...
	def list_collections(self) -> List[str]:
		...
		try:
			...
			logger.error(f'LangChainQdrantService - Failed to list collections: {str(e)}', exc_info=True)
			return []
	...
	def collection_exists(self, collection_name: str) -> bool:
		...
		try:
			...
			logger.error(f'LangChainQdrantService - Error checking collection: {str(e)}', exc_info=True)
			return False
	...
	def create_collection(self, collection_name: str) -> bool:
		...
		try:
			...
			logger.error(f'LangChainQdrantService - Failed to create collection: {str(e)}', exc_info=True)
			return False
	...
	async def index_conversation_files(self, conversation_id: str, files_data: List[Dict[str, Any]]) -> Dict[str, Any]:
		...
		try:
			...
			logger.error(f'LangChainQdrantService - Failed to index conversation files: {str(e)}', exc_info=True)
	...
	async def search_conversation_files(
		...
		try:
			...
			logger.error(f'LangChainQdrantService - Failed to search conversation files: {str(e)}', exc_info=True)
			return []
