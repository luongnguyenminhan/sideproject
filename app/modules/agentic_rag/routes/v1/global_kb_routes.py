...
from app.modules.agentic_rag.schemas.global_kb_request import (
    CreateGlobalKBRequest, UpdateGlobalKBRequest, SearchGlobalKBRequest
)
...
def get_document(doc_id: str, repo: GlobalKBRepo = Depends()):
    ...
    return APIResponse(
        error_code=0,
        message=_('success'),
        data=GlobalKBResponse.model_validate(doc)
    )
...
@handle_exceptions
def update_document(doc_id: str, request: UpdateGlobalKBRequest, repo: GlobalKBRepo = Depends()):
...
@handle_exceptions
def search_documents(request: SearchGlobalKBRequest = Depends(), repo: GlobalKBRepo = Depends()):
