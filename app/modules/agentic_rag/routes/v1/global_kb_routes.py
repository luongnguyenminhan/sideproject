...
async def list_documents(repo: GlobalKBRepo = Depends()):
    ...
    try:
        ...
        logger.debug(
            f"[GlobalKBRoutes] Converted {len(response_data)} documents to response format"
        )
...
async def get_document(doc_id: str, repo: GlobalKBRepo = Depends()):
    ...
    try:
        logger.debug(
            f"[GlobalKBRoutes] Calling repo.get_document with doc_id: {doc_id}"
        )
        doc = repo.get_document(doc_id)
        logger.info(
            f'[GlobalKBRoutes] Successfully retrieved document: {doc.title if hasattr(doc, "title") else "Unknown"}'
        )

        logger.debug("[GlobalKBRoutes] Converting document to response format")
        response_data = GlobalKBResponse.model_validate(doc)
        logger.debug(
            "[GlobalKBRoutes] Successfully converted document to response format"
        )

        logger.info(
            f"[GlobalKBRoutes] Completed get_document request for doc_id: {doc_id}"
        )
        return APIResponse(error_code=0, message=_("success"), data=response_data)
    except Exception as e:
        logger.error(
            f"[GlobalKBRoutes] Error in get_document for doc_id {doc_id}: {str(e)}"
        )
        raise
...
async def create_document(
    request: CreateGlobalKBRequest, repo: GlobalKBRepo = Depends()
):
    ...
    try:
        ...
        logger.debug(
            f"[GlobalKBRoutes] Dumped request data keys: {list(request_data.keys())}"
        )

        logger.debug("[GlobalKBRoutes] Calling repo.create_document")
        doc = repo.create_document(request_data)
        logger.info(
            f'[GlobalKBRoutes] Successfully created document with ID: {doc.id if hasattr(doc, "id") else "Unknown"}'
        )

        logger.debug("[GlobalKBRoutes] Converting created document to response format")
        response_data = GlobalKBResponse.model_validate(doc)
        logger.debug(
            "[GlobalKBRoutes] Successfully converted created document to response format"
        )
...
async def update_document(
    ...
    logger.info(
        f"[GlobalKBRoutes] Starting update_document request for doc_id: {doc_id}"
    )
    try:
        logger.debug(
            f"[GlobalKBRoutes] Request data: {request.model_dump(exclude_unset=True)}"
        )
        request_data = request.model_dump(exclude_unset=True)
        logger.debug(f"[GlobalKBRoutes] Update data keys: {list(request_data.keys())}")

        logger.debug(
            f"[GlobalKBRoutes] Calling repo.update_document for doc_id: {doc_id}"
        )
        doc = repo.update_document(doc_id, request_data)
        logger.info(
            f'[GlobalKBRoutes] Successfully updated document: {doc.title if hasattr(doc, "title") else "Unknown"}'
        )

        logger.debug("[GlobalKBRoutes] Converting updated document to response format")
        response_data = GlobalKBResponse.model_validate(doc)
        logger.debug(
            "[GlobalKBRoutes] Successfully converted updated document to response format"
        )

        logger.info(
            f"[GlobalKBRoutes] Completed update_document request for doc_id: {doc_id}"
        )
        return APIResponse(error_code=0, message=_("success"), data=response_data)
    except Exception as e:
        logger.error(
            f"[GlobalKBRoutes] Error in update_document for doc_id {doc_id}: {str(e)}"
        )
        raise
...
async def delete_document(doc_id: str, repo: GlobalKBRepo = Depends()):
    logger.info(
        f"[GlobalKBRoutes] Starting delete_document request for doc_id: {doc_id}"
    )
    try:
        logger.debug(
            f"[GlobalKBRoutes] Calling repo.delete_document for doc_id: {doc_id}"
        )
        repo.delete_document(doc_id)
        logger.info(
            f"[GlobalKBRoutes] Successfully deleted document with doc_id: {doc_id}"
        )

        logger.info(
            f"[GlobalKBRoutes] Completed delete_document request for doc_id: {doc_id}"
        )
        return APIResponse(error_code=0, message=_("success"), data=None)
    except Exception as e:
        logger.error(
            f"[GlobalKBRoutes] Error in delete_document for doc_id {doc_id}: {str(e)}"
        )
        raise
...
async def search_documents(
    request: SearchGlobalKBRequest, repo: GlobalKBRepo = Depends()
):
    logger.info(f"[GlobalKBRoutes] Starting search_documents request")
    try:
        logger.debug(
            f"[GlobalKBRoutes] Search query: '{request.query}', top_k: {request.top_k}, category: {request.category}"
        )
        ...
        response_data = [GlobalKBResponse.model_validate(doc) for doc in docs]
        logger.debug(
            f"[GlobalKBRoutes] Converted {len(response_data)} search results to response format"
        )

        logger.info(
            f"[GlobalKBRoutes] Completed search_documents request with {len(response_data)} results"
        )
        return APIResponse(
            error_code=0,
            message=_("success"),
...
async def upload_global_kb_file(
    ...
    logger.debug(
        f"[GlobalKBRoutes] File details - filename: {file.filename}, content_type: {file.content_type}, title: {title}, category: {category}"
    )
    ...
    try:
        ...
        logger.info(
            f"[GlobalKBRoutes] Successfully uploaded file to MinIO with object_name: {object_name}"
        )

        logger.debug(
            f"[GlobalKBRoutes] Getting file URL from MinIO for object_name: {object_name}"
        )
        ...
        logger.info(
            f'[GlobalKBRoutes] Successfully created document record with ID: {doc.id if hasattr(doc, "id") else "Unknown"}'
        )
        ...
        try:
            ...
            logger.debug(
                f"[GlobalKBRoutes] Read {len(file_content)} bytes from uploaded file"
            )
            ...
            logger.debug(
                "[GlobalKBRoutes] Calling file_extraction_service.extract_text_from_file"
            )
            ...
            logger.info(
                f'[GlobalKBRoutes] Text extraction result - Success: {extraction_result["extraction_success"]}, Content length: {extraction_result["char_count"]}'
            )

            if (
                extraction_result["extraction_success"]
                and extraction_result["content"].strip()
            ):
                extracted_content = extraction_result["content"]
                logger.info(
                    f"[GlobalKBRoutes] Successfully extracted text content ({len(extracted_content)} characters)"
                )

                # Index vào Global KB vector database
                logger.debug(
                    "[GlobalKBRoutes] Starting indexing to Global KB vector database"
                )
                ...
                logger.debug(
                    "[GlobalKBRoutes] Calling global_kb_service.index_admin_documents"
                )
                indexing_result = await global_kb_service.index_admin_documents(
                    documents_data
                )
                ...
                if indexing_result["successful_docs"]:
                    logger.info(
                        "[GlobalKBRoutes] Successfully indexed document, updating status"
                    )
                    repo.update_document(
                        str(doc.id), {"indexed": True, "index_status": "success"}
                    )
                else:
                    logger.warning(
                        "[GlobalKBRoutes] Failed to index document, updating status"
                    )
                    repo.update_document(
                        str(doc.id), {"indexed": False, "index_status": "failed"}
                    )

            else:
                logger.warning(
                    f'[GlobalKBRoutes] Text extraction failed: {extraction_result.get("extraction_error", "Unknown error")}'
                )
        ...
        logger.debug(
            "[GlobalKBRoutes] Successfully converted uploaded document to response format"
        )

        logger.info(
            f"[GlobalKBRoutes] Completed upload_global_kb_file request successfully"
        )
        api_response = APIResponse(
            error_code=0, message=_("success"), data=response_data
        )
        return api_response
...
async def delete_global_kb_file(doc_id: str, repo: GlobalKBRepo = Depends()):
    logger.info(
        f"[GlobalKBRoutes] Starting delete_global_kb_file request for doc_id: {doc_id}"
    )
    try:
        logger.debug(f"[GlobalKBRoutes] Getting document details for doc_id: {doc_id}")
        doc = repo.get_document(doc_id)
        logger.info(
            f'[GlobalKBRoutes] Retrieved document: {doc.title if hasattr(doc, "title") else "Unknown"}'
        )
        ...
        if doc and doc.source:
            ...
            logger.info(
                f"[GlobalKBRoutes] Extracted object_name for deletion: {object_name}"
            )

            logger.debug(f"[GlobalKBRoutes] Removing file from MinIO: {object_name}")
            minio_handler.remove_file(object_name)
            logger.info(
                f"[GlobalKBRoutes] Successfully removed file from MinIO: {object_name}"
            )
        else:
            logger.warning(
                f"[GlobalKBRoutes] Document {doc_id} has no source file or document not found"
            )

        logger.debug(f"[GlobalKBRoutes] Deleting document from database: {doc_id}")
        repo.delete_document(doc_id)
        logger.info(
            f"[GlobalKBRoutes] Successfully deleted document from database: {doc_id}"
        )

        logger.info(
            f"[GlobalKBRoutes] Completed delete_global_kb_file request for doc_id: {doc_id}"
        )
        return APIResponse(error_code=0, message=_("success"), data=None)
    except Exception as e:
        logger.error(
            f"[GlobalKBRoutes] Error in delete_global_kb_file for doc_id {doc_id}: {str(e)}"
        )
        raise
