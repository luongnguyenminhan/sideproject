...
class CVRepo:
    ...
    async def process_cv(self, request: ProcessCVRequest) -> APIResponse:
        ...
        try:
            ...
            file_content, filename = await n8n_client.download_file_content(
                request.cv_file_url
            )
            if not file_content:
    ...
    def _build_success_response(
        ...
        try:
            ...
            experience_items = result.get("work_experience_history", {}).get(
                "items", []
            )
            ...
            response_data = ProcessCVResponse(
                ...
                experience_count=(
                    len(experience_items) if isinstance(experience_items, list) else 0
                ),
