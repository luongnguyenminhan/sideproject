...
from ..schemas.facebook_schema import (
    FacebookPageInfo,
    FacebookPost,
)
    ...
    async def get_page_info_with_posts(self, limit: int = 5) -> FacebookPageInfo:
        ...
        if cached_data:
            ...
            try:
                ...
                logger.info(
                    f"\033[93mWARNING: Failed to validate cached data: {e}\033[0m"
                )
                await redis_client.delete(cache_key)

        # If not in cache, fetch from API
        logger.info(
            f"\033[94mINFO: No valid cache found, fetching from Facebook API\033[0m"
        )
        try:
            ...
            if isinstance(e, (CustomHTTPException, NotFoundException)):
                raise e
            logger.info(
                f"\033[91mERROR: Unexpected error in Facebook API call: {e}\033[0m"
            )
