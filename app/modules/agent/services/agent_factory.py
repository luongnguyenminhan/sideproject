...
and complete tasks efficiently. You provide structured and actionable guidance.""",
	...
	def create_default_agent(cls, agent_type: AgentType, user_id: str, agent_repo: 'AgentRepo', custom_name: str = None) -> Agent:
		...
		print('\033[92m[DEBUG] AgentFactory.create_default_agent() - ENTRY\033[0m')
		print(f'\033[94m[DEBUG] Parameters: agent_type={agent_type}, user_id={user_id}, custom_name={custom_name}\033[0m')

		if agent_type not in cls.DEFAULT_CONFIGS:
			print(f'\033[91m[DEBUG] ERROR: Unsupported agent type: {agent_type}\033[0m')
			raise ValidationException(_('unsupported_agent_type'))

		print(f'\033[93m[DEBUG] Agent type {agent_type} found in DEFAULT_CONFIGS\033[0m')

		# Get default config for agent type
		default_config = cls.DEFAULT_CONFIGS[agent_type].copy()
		print(f'\033[96m[DEBUG] Retrieved default config: {default_config["name"]}\033[0m')

		# Use custom name if provided
		if custom_name:
			print(f'\033[95m[DEBUG] Using custom name: {custom_name}\033[0m')
			default_config['name'] = custom_name
		else:
			print(f'\033[95m[DEBUG] No custom name provided, using default: {default_config["name"]}\033[0m')

		# Create or get existing default config
		print('\033[93m[DEBUG] Calling _get_or_create_default_config()\033[0m')
		config = cls._get_or_create_default_config(agent_type, default_config, agent_repo)
		print(f'\033[92m[DEBUG] Config created/retrieved with ID: {config.id}\033[0m')

		# Create agent
		agent_name = custom_name or f'{default_config["name"]} - {user_id[:8]}'
		print(f'\033[94m[DEBUG] Final agent name: {agent_name}\033[0m')
		print(f'\033[94m[DEBUG] Agent description: {default_config["description"]}\033[0m')

		print('\033[93m[DEBUG] Calling agent_repo.create_agent()\033[0m')
		...
		print(f'\033[92m[DEBUG] Agent created successfully with ID: {agent.id}\033[0m')
		print('\033[92m[DEBUG] AgentFactory.create_default_agent() - EXIT\033[0m')
	...
	def create_custom_agent(cls, agent_type: AgentType, user_id: str, agent_repo: 'AgentRepo', custom_config: Dict[str, Any], agent_name: str) -> Agent:
		...
		print('\033[92m[DEBUG] AgentFactory.create_custom_agent() - ENTRY\033[0m')
		print(f'\033[94m[DEBUG] Parameters: agent_type={agent_type}, user_id={user_id}, agent_name={agent_name}\033[0m')
		print(f'\033[94m[DEBUG] Custom config keys: {list(custom_config.keys())}\033[0m')

		# Validate custom config
		print('\033[93m[DEBUG] Validating custom config\033[0m')
		validated_config = cls._validate_custom_config(agent_type, custom_config)
		print('\033[92m[DEBUG] Custom config validation passed\033[0m')

		# Create custom config
		print('\033[93m[DEBUG] Creating custom config\033[0m')
		config = cls._create_custom_config(agent_type, validated_config, agent_repo)
		print(f'\033[92m[DEBUG] Custom config created with ID: {config.id}\033[0m')

		# Create agent
		description = custom_config.get('description', f'Custom {agent_type.value} agent')
		print(f'\033[94m[DEBUG] Agent description: {description}\033[0m')

		print('\033[93m[DEBUG] Calling agent_repo.create_agent() for custom agent\033[0m')
		...
		print(f'\033[92m[DEBUG] Custom agent created successfully with ID: {agent.id}\033[0m')
		print('\033[92m[DEBUG] AgentFactory.create_custom_agent() - EXIT\033[0m')
	...
	def get_default_config_template(cls, agent_type: AgentType) -> Dict[str, Any]:
		...
		print(f'\033[92m[DEBUG] AgentFactory.get_default_config_template() - agent_type={agent_type}\033[0m')

		if agent_type not in cls.DEFAULT_CONFIGS:
			print(f'\033[91m[DEBUG] ERROR: Unsupported agent type in template: {agent_type}\033[0m')
			raise ValidationException(_('unsupported_agent_type'))

		template = cls.DEFAULT_CONFIGS[agent_type].copy()
		print(f'\033[94m[DEBUG] Returning template for {agent_type}: {template["name"]}\033[0m')
		return template

	@classmethod
	def list_available_models(cls) -> Dict[str, List[str]]:
		"""List available models by provider"""
		print('\033[92m[DEBUG] AgentFactory.list_available_models() - ENTRY\033[0m')
		models = {
			ModelProvider.GOOGLE.value: ['gemini-2.0-flash', 'gemini-2.0-flash-lite', 'gemini-2.5-flash'],
		}
		print(f'\033[94m[DEBUG] Available models: {models}\033[0m')
		return models
	...
	def validate_model_compatibility(cls, provider: ModelProvider, model_name: str) -> bool:
		...
		print(f'\033[92m[DEBUG] AgentFactory.validate_model_compatibility() - provider={provider}, model={model_name}\033[0m')

		available_models = cls.list_available_models()
		compatible = model_name in available_models.get(provider.value, [])

		print(f'\033[94m[DEBUG] Model compatibility result: {compatible}\033[0m')
		return compatible
	...
	def _get_or_create_default_config(cls, agent_type: AgentType, default_config: Dict[str, Any], agent_repo: 'AgentRepo') -> AgentConfig:
		...
		print('\033[92m[DEBUG] AgentFactory._get_or_create_default_config() - ENTRY\033[0m')
		print(f'\033[94m[DEBUG] Looking for config: default_{agent_type.value}_config\033[0m')

		try:
			# Try to find existing default config
			config_name = f'default_{agent_type.value}_config'
			print(f'\033[93m[DEBUG] Searching for existing config: {config_name}\033[0m')

			existing_config = agent_repo.config_dal.get_config_by_name(config_name)

			if existing_config:
				print(f'\033[92m[DEBUG] Found existing config with ID: {existing_config.id}\033[0m')
				return existing_config

			print('\033[93m[DEBUG] No existing config found, creating new default config\033[0m')
			...
			print(f'\033[96m[DEBUG] Config data prepared: name={config_data["name"]}, provider={config_data["model_provider"]}, model={config_data["model_name"]}\033[0m')
			print(f'\033[96m[DEBUG] Temperature: {config_data["temperature"]}, Max tokens: {config_data["max_tokens"]}\033[0m')

			print('\033[93m[DEBUG] Calling agent_repo.config_dal.create()\033[0m')
			created_config = agent_repo.config_dal.create(config_data)

			if not created_config:
				print('\033[91m[DEBUG] ERROR: Failed to create default config - config_dal.create() returned None\033[0m')
				raise ValidationException(_('failed_to_create_default_config'))

			print(f'\033[92m[DEBUG] Default config created successfully with ID: {created_config.id}\033[0m')
			print('\033[92m[DEBUG] AgentFactory._get_or_create_default_config() - EXIT\033[0m')
			return created_config

		except Exception as e:
			print(f'\033[91m[DEBUG] ERROR in _get_or_create_default_config(): {str(e)}\033[0m')
			print(f'\033[91m[DEBUG] Exception type: {type(e).__name__}\033[0m')
	...
	def _create_custom_config(cls, agent_type: AgentType, custom_config: Dict[str, Any], agent_repo: 'AgentRepo') -> AgentConfig:
		...
		print('\033[92m[DEBUG] AgentFactory._create_custom_config() - ENTRY\033[0m')
		print(f'\033[94m[DEBUG] Agent type: {agent_type}\033[0m')
		...
		print(f'\033[96m[DEBUG] Custom config data: name={config_data["name"]}, provider={config_data["model_provider"]}\033[0m')
		print(f'\033[96m[DEBUG] Model: {config_data["model_name"]}, Temperature: {config_data["temperature"]}\033[0m')

		print('\033[93m[DEBUG] Calling agent_repo.config_dal.create() for custom config\033[0m')
		config = agent_repo.config_dal.create(config_data)
		print(f'\033[92m[DEBUG] Custom config created with ID: {config.id}\033[0m')
		print('\033[92m[DEBUG] AgentFactory._create_custom_config() - EXIT\033[0m')
	...
	def _validate_custom_config(cls, agent_type: AgentType, custom_config: Dict[str, Any]) -> Dict[str, Any]:
		...
		print('\033[92m[DEBUG] AgentFactory._validate_custom_config() - ENTRY\033[0m')
		print(f'\033[94m[DEBUG] Validating config for agent type: {agent_type}\033[0m')
		print(f'\033[94m[DEBUG] Config keys to validate: {list(custom_config.keys())}\033[0m')

		# Required fields
		required_fields = ['model_provider', 'model_name']
		print(f'\033[93m[DEBUG] Checking required fields: {required_fields}\033[0m')

		for field in required_fields:
			if field not in custom_config:
				print(f'\033[91m[DEBUG] ERROR: Missing required field: {field}\033[0m')
		...
		print(f'\033[93m[DEBUG] Validating model provider: {custom_config["model_provider"]}\033[0m')
		try:
			provider = ModelProvider(custom_config['model_provider'])
			print(f'\033[92m[DEBUG] Model provider validation passed: {provider}\033[0m')
		except ValueError:
			print(f'\033[91m[DEBUG] ERROR: Invalid model provider: {custom_config["model_provider"]}\033[0m')
			raise ValidationException(_('invalid_model_provider'))

		# Validate model compatibility
		print(f'\033[93m[DEBUG] Validating model compatibility: {custom_config["model_name"]}\033[0m')
		if not cls.validate_model_compatibility(provider, custom_config['model_name']):
			print(f'\033[91m[DEBUG] ERROR: Invalid model for provider: {custom_config["model_name"]}\033[0m')
			raise ValidationException(_('invalid_model_for_provider'))
		print('\033[92m[DEBUG] Model compatibility validation passed\033[0m')

		# Validate temperature
		temperature = custom_config.get('temperature', 0.7)
		print(f'\033[93m[DEBUG] Validating temperature: {temperature}\033[0m')
		if not 0 <= temperature <= 2:
			print(f'\033[91m[DEBUG] ERROR: Invalid temperature range: {temperature}\033[0m')
			raise ValidationException(_('invalid_temperature_range'))
		print('\033[92m[DEBUG] Temperature validation passed\033[0m')

		# Validate max_tokens
		max_tokens = custom_config.get('max_tokens', 2048)
		print(f'\033[93m[DEBUG] Validating max_tokens: {max_tokens}\033[0m')
		if not 1 <= max_tokens <= 32000:
			print(f'\033[91m[DEBUG] ERROR: Invalid max_tokens range: {max_tokens}\033[0m')
			raise ValidationException(_('invalid_max_tokens_range'))
		print('\033[92m[DEBUG] Max tokens validation passed\033[0m')

		print('\033[92m[DEBUG] All validations passed successfully\033[0m')
		print('\033[92m[DEBUG] AgentFactory._validate_custom_config() - EXIT\033[0m')
