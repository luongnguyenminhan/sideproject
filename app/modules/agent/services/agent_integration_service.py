...
class AgentIntegrationService:
    ...
    async def get_ai_response(self, conversation_id: str, user_message: str, 
                            user_id: str, api_key: str = None) -> Dict[str, Any]:
        ...
        # Execute workflow
        result = await self.workflow_repo.execute_chat_workflow(
            agent_id=agent.id,
            user_id=user_id,
    ...
    async def get_ai_response_streaming(self, conversation_id: str, user_message: str,
                                      user_id: str, api_key: str = None,
        ...
        try:
            ...
            async for chunk in self.workflow_repo.execute_streaming_workflow(
                agent_id=agent.id,
                user_id=user_id,
                conversation_id=conversation_id,
                user_message=user_message,
                ...
                if chunk.get('type') == 'content':
                    ...
                    if websocket_manager and user_id:
                        await websocket_manager.send_message(
                            ...
                            {
                                'type': 'error',
                                'message': chunk.get('message', 'Workflow execution failed')
                            }
                        )
    ...
    async def test_agent_response(self, agent_id: str, user_id: str, 
                                test_message: str, api_key: str = None) -> Dict[str, Any]:
    ...
    def get_agent_conversation_context(self, agent_id: str, user_id: str, 
        ...
        # Get memory context
        context = self.workflow_repo.get_agent_memory_context(
            agent_id=agent_id,
            conversation_id=conversation_id
        )
    ...
    def clear_agent_conversation_memory(self, agent_id: str, user_id: str,
        ...
        return self.workflow_repo.memory_dal.clear_conversation_memories(
    ...
    def get_agent_statistics(self, agent_id: str, user_id: str) -> Dict[str, Any]:
        ...
        # Get workflow state
        workflow_memories = self.workflow_repo.memory_dal.get_memories_by_agent(
            agent_id, memory_type=self.workflow_repo.memory_dal.model.MemoryType.WORKFLOW_STATE, limit=1
        )
