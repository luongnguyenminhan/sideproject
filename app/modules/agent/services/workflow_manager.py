...
class WorkflowManager:
	...
	def _validate_chat_config(self, config: AgentConfig) -> bool:
		...
		return (
			config.temperature >= 0.5  # More creative for conversation
			and config.max_tokens >= 512  # Sufficient for responses
			and 'conversational' in config.system_prompt.lower()
			if config.system_prompt
			else True
		)
	...
	def _validate_analysis_config(self, config: AgentConfig) -> bool:
		...
		return (
			config.temperature <= 0.5  # More precise for analysis
			and config.max_tokens >= 1024  # More space for detailed analysis
			and 'analys' in config.system_prompt.lower()
			if config.system_prompt
			else True
		)
	...
	def _validate_task_config(self, config: AgentConfig) -> bool:
		...
		return (
			config.temperature <= 0.7  # Balanced for task guidance
			and config.max_tokens >= 512  # Sufficient for task instructions
			and 'task' in config.system_prompt.lower()
			if config.system_prompt
			else True
		)
