...
class ConversationFileIndexingService:
    ...
    async def index_conversation_files(
        self, conversation_id: str, files_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            logger.info(
                f"[FileIndexingService] Starting Agentic RAG indexing {len(files_data)} files for conversation: {conversation_id}"
            )
                    ...
                    if (
                        ...
                        doc = DocumentModel(
                            ...
                            metadata={
                                ...
                                "original_file_id": file_data[
                                    "file_id"
                                ],  # Lưu original file_id để reference
                            },
                        )
                        ...
                        logger.info(
                            f'[FileIndexingService] Extracted {extraction_result["char_count"]} chars from {file_data["file_name"]}'
                        )
                    else:
                        failed_files.append(
                            {
                        ...
                        logger.warning(
                            f'[FileIndexingService] Failed to extract text from {file_data["file_name"]}'
                        )

                except Exception as e:
                    logger.error(
                        f'[FileIndexingService] Error processing file {file_data["file_name"]}: {str(e)}'
                    )
                    failed_files.append(
                        {"file_id": file_data["file_id"], "error": str(e)}
                    )
            ...
            if documents_to_add:
                try:
                    ...
                    document_ids = await kb_repo.add_documents(
                        add_request, collection_id=collection_id
                    )
                    indexing_result = {
                    ...
                    logger.info(
                        f"[FileIndexingService] Successfully indexed {len(document_ids)} documents to Agentic RAG collection: {collection_id}"
                    )

                except Exception as e:
                    logger.error(
                        f"[FileIndexingService] Error indexing documents to Agentic RAG: {str(e)}"
                    )
                    # Move successful files to failed
                    for file_id in successful_files:
                        failed_files.append(
                            {"file_id": file_id, "error": f"Indexing failed: {str(e)}"}
                        )
                    successful_files = []
                    indexing_result = {"error": str(e)}
            else:
                logger.warning(
                    f"[FileIndexingService] No documents to index for conversation: {conversation_id}"
                )
            ...
            logger.error(
                f"[FileIndexingService] Error indexing conversation files via Agentic RAG: {str(e)}"
            )
    ...
    def search_conversation_context(
        ...
        try:
            ...
            logger.info(
                f"[FileIndexingService] Found {len(documents)} relevant documents via Agentic RAG for query in conversation: {conversation_id}"
            )
            return documents

        except Exception as e:
            logger.error(
                f"[FileIndexingService] Error searching conversation context via Agentic RAG: {str(e)}"
            )
    ...
    def get_conversation_collection_stats(self, conversation_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"[FileIndexingService] Error getting Agentic RAG collection stats: {str(e)}"
            )
            return {
                "name": collection_id,
                "documents_count": 0,
    ...
    def delete_conversation_collection(self, conversation_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"[FileIndexingService] Successfully deleted {len(documents)} documents from Agentic RAG collection: {collection_id}"
            )
            return True

        except Exception as e:
            logger.error(
                f"[FileIndexingService] Error deleting Agentic RAG conversation collection: {str(e)}"
            )
            return False
    ...
    def check_collection_exists(self, conversation_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"[FileIndexingService] Error checking Agentic RAG collection existence: {str(e)}"
            )
            return False
