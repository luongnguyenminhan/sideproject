...
class SurveyResponseProcessor:
	...
	@staticmethod
	def process_survey_response(survey_data: Dict[str, Any], conversation_id: str, user_id: str, processing_type: str = 'analysis') -> str:
		"""
		...
		try:
			...
			formatted_response = SurveyResponseProcessor._format_survey_for_ai(answers=answers, processing_type=processing_type, conversation_id=conversation_id)
	...
	@staticmethod
	def _format_survey_for_ai(answers: Dict[str, Any], processing_type: str, conversation_id: str) -> str:
		...
		formatted_parts = [intro, '', '**Survey Response Details:**', f'- Conversation ID: {conversation_id}', f'- Total Questions Answered: {len(answers)}', f'- Response Date: {SurveyResponseProcessor._get_current_timestamp()}', '', '**Responses:**']

		# Format each answer
		for question_id, answer_value in answers.items():
			formatted_answer = SurveyResponseProcessor._format_individual_answer(question_id, answer_value)
			formatted_parts.append(formatted_answer)
			formatted_parts.append('')

		# Add analysis instructions
		formatted_parts.extend(['---', '**Analysis Instructions:**', '- Provide thoughtful analysis of the responses', '- Identify patterns or themes in the answers', '- Offer relevant insights or recommendations', '- Maintain a helpful and empathetic tone', '- Ask clarifying questions if needed'])
...
def survey_response_tool(input_data: str) -> str:
	...
	try:
		...
		result = SurveyResponseProcessor.process_survey_response(survey_data=survey_data, conversation_id=conversation_id, user_id=user_id, processing_type=processing_type)
