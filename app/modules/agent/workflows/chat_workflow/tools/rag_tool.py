...
def rag_retrieval_tool(conversation_id: str, user_id: str, query: str, top_k: int = 5) -> str:
	"""
...
async def _execute_rag_async(conversation_id: str, user_id: str, query: str, top_k: int) -> str:
...
async def _execute_retrieval_async(conversation_id: str, user_id: str, route_decision: RAGRoute, top_k: int) -> Dict[str, List[Dict]]:
	...
	try:
		...
		if route_decision.routing_decision in ['global_only', 'both']:
			global_results = await _search_global_kb_async(route_decision.enhanced_query, top_k)
		...
		if route_decision.routing_decision in ['conversation_only', 'both']:
			conv_results = await _search_conversation_kb_async(conversation_id, user_id, route_decision.enhanced_query, top_k)
...
async def _search_global_kb_async(query: str, top_k: int) -> List[Dict[str, Any]]:
	...
	try:
		...
		if not query or not query.strip():
			logger.warning('[RAG] Empty query provided for global KB search')
			return []

		if top_k <= 0:
			logger.warning(f'[RAG] Invalid top_k value: {top_k}, using default 3')
			top_k = 3

		# TODO: Replace with actual implementation
		# For now, return improved mock with better error handling
		try:
			...
			mock_results = [
				{
					'content': f'Thông tin toàn cầu về {query} - kết quả tìm kiếm chất lượng cao',
					'metadata': {
						'source': 'global_kb',
					...
					'source': 'global_kb',
					'doc_id': f'global_{abs(hash(query)) % 10000}',
				},
				{
					'content': f'Thông tin bổ sung về {query} từ cơ sở dữ liệu toàn cầu',
					'metadata': {
						'source': 'global_kb',
					...
					'source': 'global_kb',
					'doc_id': f'global_{abs(hash(query + "alt")) % 10000}',
				},
			]
			...
			logger.info(f'[RAG] Global KB search returned {len(results)} results')
			return results

		except Exception as search_error:
			logger.error(f'[RAG] Error in global KB search execution: {str(search_error)}')
			return []
		...
		logger.error(f'[RAG] Critical error in global KB search: {str(e)}')
		return []


async def _search_conversation_kb_async(conversation_id: str, user_id: str, query: str, top_k: int) -> List[Dict[str, Any]]:
...
async def _get_cv_context_async(conversation_id: str, user_id: str, query: str) -> Optional[str]:
...
def _combine_and_rank_results(results: Dict[str, List[Dict]], route_decision: RAGRoute) -> Dict[str, Any]:
