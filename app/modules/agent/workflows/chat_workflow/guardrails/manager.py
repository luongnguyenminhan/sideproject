...
class ChatWorkflowGuardrailManager:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.enable_output_guardrails = self.config.get(
        ...
        if self.enable_llm_guardrails:
            self.engine = LLMGuardrailEngine(
                enable_llm_guardrails=True, model_name=self.model_name
            )
            color_logger.info(
    ...
    def _setup_traditional_guardrails(self):
        ...
        if self.enable_input_guardrails:
            ...
            self.engine.add_input_guardrail(
                LengthGuardrail(max_length=self.max_input_length)
            )
    ...
    def check_ai_output(
        self, ai_output: str, context: Dict[str, Any] = None
    ) -> GuardrailResult:
        """
    ...
    def get_active_guardrails(self) -> Dict[str, List[str]]:
        ...
        active_output = [
            g.name for g in self.engine.output_guardrails if g.is_enabled()
        ]
...
def create_hybrid_manager(
    model_name: str = "gemini-2.0-flash-lite", strict_mode: bool = False
) -> ChatWorkflowGuardrailManager:
