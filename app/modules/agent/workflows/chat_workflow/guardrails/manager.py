...
class ChatWorkflowGuardrailManager:
	...
	def __init__(self, config: Dict[str, Any] = None):
		...
		if self.enable_llm_guardrails:
			...
			color_logger.info(f'🧠 {Colors.BOLD}LLM GUARDRAIL MANAGER:{Colors.RESET} Initialized with AI-powered protection', Colors.BRIGHT_GREEN, llm_enabled=True, model=self.model_name, use_llm_only=self.use_llm_only)
		else:
			self.engine = GuardrailEngine()
			color_logger.info(f'🛡️ {Colors.BOLD}TRADITIONAL GUARDRAIL MANAGER:{Colors.RESET} Initialized with rule-based protection', Colors.BRIGHT_BLUE, llm_enabled=False)
	...
	def _setup_traditional_guardrails(self):
		...
		color_logger.info(f'⚙️ {Colors.BOLD}SETTING UP TRADITIONAL GUARDRAILS:{Colors.RESET} Adding rule-based protection', Colors.BRIGHT_CYAN, complement_llm=self.enable_llm_guardrails)
	...
	def check_user_input(self, user_input: str, context: Dict[str, Any] = None) -> GuardrailResult:
		...
		enhanced_context.update({'analysis_type': 'input', 'strict_mode': self.strict_mode, 'timestamp': datetime.now().isoformat(), 'llm_enabled': self.enable_llm_guardrails})

		color_logger.workflow_start('User Input Guardrail Check', llm_enabled=self.enable_llm_guardrails, content_length=len(user_input), traditional_guards=not self.use_llm_only)

		result = self.engine.check_input(user_input, enhanced_context)

		color_logger.info(f'🛡️ {Colors.BOLD}INPUT GUARDRAIL RESULT:{Colors.RESET} {"✅ PASSED" if result.passed else "❌ BLOCKED"}', Colors.BRIGHT_GREEN if result.passed else Colors.BRIGHT_RED, violations_count=len(result.violations), modified=bool(result.modified_content), processing_time=result.processing_time)
	...
	def check_ai_output(self, ai_output: str, context: Dict[str, Any] = None) -> GuardrailResult:
		...
		# Enhanced context for LLM analysis
		enhanced_context = context or {}
		enhanced_context.update({'analysis_type': 'output', 'strict_mode': self.strict_mode, 'timestamp': datetime.now().isoformat(), 'llm_enabled': self.enable_llm_guardrails})

		color_logger.workflow_start('AI Output Guardrail Check', llm_enabled=self.enable_llm_guardrails, content_length=len(ai_output), traditional_guards=not self.use_llm_only)

		result = self.engine.check_output(ai_output, enhanced_context)

		color_logger.info(f'🛡️ {Colors.BOLD}OUTPUT GUARDRAIL RESULT:{Colors.RESET} {"✅ PASSED" if result.passed else "⚠️ FLAGGED"}', Colors.BRIGHT_GREEN if result.passed else Colors.BRIGHT_YELLOW, violations_count=len(result.violations), modified=bool(result.modified_content), processing_time=result.processing_time)
	...
	def get_guardrail_stats(self) -> Dict[str, Any]:
		...
		stats.update({'llm_enabled': self.enable_llm_guardrails, 'use_llm_only': self.use_llm_only, 'model_name': self.model_name if self.enable_llm_guardrails else None, 'manager_config': {'enable_input_guardrails': self.enable_input_guardrails, 'enable_output_guardrails': self.enable_output_guardrails, 'strict_mode': self.strict_mode, 'max_input_length': self.max_input_length}})
		return stats
	...
	def disable_guardrail(self, guardrail_name: str) -> bool:
		...
		for guardrail in self.engine.input_guardrails + self.engine.output_guardrails:
			if guardrail.name == guardrail_name:
				...
				color_logger.info(f'🚫 {Colors.BOLD}GUARDRAIL DISABLED:{Colors.RESET} {guardrail_name}', Colors.BRIGHT_YELLOW)
				return True
		return False
	...
	def enable_guardrail(self, guardrail_name: str) -> bool:
		...
		for guardrail in self.engine.input_guardrails + self.engine.output_guardrails:
			if guardrail.name == guardrail_name:
				...
				color_logger.info(f'✅ {Colors.BOLD}GUARDRAIL ENABLED:{Colors.RESET} {guardrail_name}', Colors.BRIGHT_GREEN)
				return True
		return False
	...
	def get_active_guardrails(self) -> Dict[str, List[str]]:
		...
		return {'input_guardrails': active_input, 'output_guardrails': active_output, 'total_active': len(active_input) + len(active_output)}


# Factory functions để tạo các loại guardrail manager khác nhau


def create_llm_only_manager(model_name: str = 'gemini-2.0-flash-lite') -> ChatWorkflowGuardrailManager:
	"""Tạo guardrail manager chỉ sử dụng LLM."""
	config = {'enable_llm_guardrails': True, 'use_llm_only': True, 'model_name': model_name, 'strict_mode': False}

	color_logger.info(f'🧠 {Colors.BOLD}CREATING LLM-ONLY GUARDRAIL MANAGER:{Colors.RESET} AI-powered protection only', Colors.BRIGHT_MAGENTA, model=model_name)
...
def create_hybrid_manager(model_name: str = 'gemini-2.0-flash-lite', strict_mode: bool = False) -> ChatWorkflowGuardrailManager:
	...
	config = {'enable_llm_guardrails': True, 'use_llm_only': False, 'model_name': model_name, 'strict_mode': strict_mode, 'enable_input_guardrails': True, 'enable_output_guardrails': True}

	color_logger.info(f'🔥 {Colors.BOLD}CREATING HYBRID GUARDRAIL MANAGER:{Colors.RESET} LLM + Traditional rules', Colors.BRIGHT_CYAN, model=model_name, strict_mode=strict_mode)

	return ChatWorkflowGuardrailManager(config)


def create_traditional_manager(strict_mode: bool = False) -> ChatWorkflowGuardrailManager:
	"""Tạo traditional guardrail manager (chỉ rule-based)."""
	config = {'enable_llm_guardrails': False, 'use_llm_only': False, 'strict_mode': strict_mode, 'enable_input_guardrails': True, 'enable_output_guardrails': True}

	color_logger.info(f'🛡️ {Colors.BOLD}CREATING TRADITIONAL GUARDRAIL MANAGER:{Colors.RESET} Rule-based protection only', Colors.BRIGHT_BLUE, strict_mode=strict_mode)
...
def get_default_guardrail_manager(config: Dict[str, Any] = None) -> ChatWorkflowGuardrailManager:
	"""
