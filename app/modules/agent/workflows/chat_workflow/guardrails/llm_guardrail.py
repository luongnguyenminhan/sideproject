...
# TODO: Consider using external color_logger module
# from ..utils.color_logger import get_color_logger, Colors
...
class LLMGuardrailDecision(BaseModel):
    ...
    action: Literal["allow", "modify", "block", "escalate"] = Field(
    ...
    modified_content: Optional[str] = Field(
        default=None, description="Nội dung đã sửa đổi (nếu có)"
    )
...
class LLMInputGuardrail(BaseGuardrail):
    ...
    def __init__(
        self, model_name: str = "gemini-2.0-flash-lite", temperature: float = 0.1
    ):
...
""",
            ...
            decision = structured_model.invoke(
                prompt.format_messages(content=content, context_info=context_info)
            )
    ...
    def _prepare_context(self, context: Dict[str, Any]) -> str:
        ...
        if context.get("previous_violations"):
            context_parts.append(
                f'Previous violations: {context["previous_violations"]}'
            )
    ...
    def _convert_to_guardrail_result(
        ...
        return GuardrailResult(
            ...
            metadata={
                ...
                "confidence": decision.confidence,
                "violation_type": (
                    decision.violation_type if decision.has_violation else None
                ),
                "tags": decision.tags,
                "original_content_length": len(original_content),
                "modified_content_length": (
                    len(decision.modified_content)
...
""",
            ...
            color_logger.error(
                f"LLM Output Guardrail Error: {str(e)}", Colors.BRIGHT_RED
            )
    ...
    def _prepare_output_context(self, context: Dict[str, Any]) -> str:
        ...
        if context.get("rag_context"):
            context_parts.append(
                f'RAG Context Available: {bool(context["rag_context"])}'
            )
        ...
        if context.get("conversation_history"):
            context_parts.append(
                f'Conversation Length: {len(context["conversation_history"])}'
            )
