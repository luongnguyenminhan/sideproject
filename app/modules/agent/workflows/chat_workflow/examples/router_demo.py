	...
	try:
		...
		for i, test_case in enumerate(TEST_QUERIES, 1):
			...
			try:
				...
				actual_target = result.target if hasattr(result, 'target') else 'unknown'
				explanation = result.explanation if hasattr(result, 'explanation') else 'No explanation'
		...
		for i, result in enumerate(results, 1):
			...
			print(f'{i}. {status} Expected: {result["expected"]} | Actual: {result["actual"]}')
...
async def test_router_node_integration():
	...
	try:
		...
		if router_decision:
			target = router_decision.get('target', 'unknown') if isinstance(router_decision, dict) else 'unknown'
			explanation = router_decision.get('explanation', 'No explanation') if isinstance(router_decision, dict) else 'No explanation'
...
if __name__ == '__main__':
	...
	async def main():
		...
		print(f'Integration Test: {"✅ Success" if integration_result else "❌ Failed"}')
