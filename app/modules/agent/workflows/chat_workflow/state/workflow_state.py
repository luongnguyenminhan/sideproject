...
class AgentState(TypedDict):
    ...
    # RAG system state (simplified from dual_rag)
    global_rag_context: Optional[List[Dict[str, Any]]]
    conversation_rag_context: Optional[List[Dict[str, Any]]]
    rag_routing: Optional[Dict[str, Any]]  # routing decision and metadata
    combined_rag_context: Optional[str]  # final combined context
...
class StateManager:
    ...
    def add_processing_time(
        state: AgentState, node_name: str, duration: float
    ) -> AgentState:
    ...
    def update_model_usage(
        ...
        # Update totals
        model_usage["total_tokens"] = sum(
            call["tokens"] for call in model_usage["calls"]
        )
        if cost:
            model_usage["total_cost"] = sum(
                call.get("cost", 0) for call in model_usage["calls"] if call.get("cost")
            )
    ...
    def extract_conversation_history(
        state: AgentState, limit: Optional[int] = None
    ) -> List[Dict[str, str]]:
