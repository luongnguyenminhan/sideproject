...
class WorkflowRouter:
    ...
    def route_after_tool_decision(self, state: AgentState) -> str:
        ...
        if decision == "use_tools":
            logger.info(
                f"[route_after_tool_decision] ✅ Routing to: use_tools (agent_with_tools)"
            )
            return "use_tools"
        else:
            logger.info(
                f"[route_after_tool_decision] ❌ Routing to: no_tools (agent_no_tools)"
            )
            return "no_tools"
    ...
    def should_continue_after_agent(self, state: AgentState) -> str:
        ...
        if hasattr(last_message, "tool_calls") and last_message.tool_calls:
            print(
                f"[should_continue_after_agent] Found {len(last_message.tool_calls)} tool calls, proceeding to tools"
            )
            return "tools"
        else:
        ...
        if not validation_result.get("is_safe", True):
            ...
            if severity in ["high", "critical"]:
                ...
                if retry_count < 2:  # Max 2 retries
                    logger.warning(
                        f"[route_after_output_validation] Critical violation detected, retrying (attempt {retry_count + 1})"
                    )
                    ...
                    print(
                        "[route_after_output_validation] Max retries exceeded, ending with safety warning"
                    )
