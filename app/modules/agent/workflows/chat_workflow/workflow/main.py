...
class Workflow:
    ...
    async def _get_rag_context(self, state: AgentState, thread_id: str) -> AgentState:
        ...
        if self.global_kb_service:
            try:
                global_results = await self.global_kb_service.search_global_knowledge(
                    user_query, top_k=2
                )
                if global_results:
                    contexts = []
        ...
        try:
            ...
            return {
                ...
                "metadata": {
                    ...
                    "persona_type": (
                        self.config.persona_type.value
                        if self.config
                        else "enterview_assistant"
                    ),
    ...
    def _count_tool_usage(self, final_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "required_tools_met": all(
                tool in tools_used for tool in final_state.get("required_tools", [])
            ),
            "triggered_rules": final_state.get("triggered_rules", []),
            "guardrails_passed": final_state.get("validation_passed", True)
            and final_state.get("response_safe", True),
        }
