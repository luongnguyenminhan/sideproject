...
class EnhancedDualRAGWorkflow:
    ...
    async def _dual_rag_processor_node(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            # Parse RAG result
            rag_data = (
                json.loads(rag_result) if isinstance(rag_result, str) else rag_result
            )
            ...
            return {
                **state,
                ...
                "global_rag_context": [
                    ...
                    if s.get("source_type") == "global"
                ],
                "conversation_rag_context": [
                    s
                    for s in rag_data.get("sources", [])
                    if s.get("source_type") == "conversation"
                ],
            ...
            return {
                ...
                "metadata": {
                    ...
                    "conversation_sources": len(
                        final_state.get("conversation_rag_context", [])
                    ),
...
def create_enhanced_dual_rag_workflow(
    db_session: Session, config: Optional[WorkflowConfig] = None
) -> EnhancedDualRAGWorkflow:
