...
class RouterDecision(BaseModel):
    ...
    target: Literal["rag_query", "direct_agent", "math_tools", "general"] = Field(
...
async def call_model(state, config):
    ...
    if workflow_config and workflow_config.persona_enabled:
        ...
        color_logger.info(
            ...
            persona_type=(
                workflow_config.persona_type.value
                if workflow_config.persona_type
                else "none"
            ),
        )
        system = persona_prompt
    else:
        system = config.get("configurable", {}).get(
...
async def router_node(state, config):
    ...
    try:
        if guardrail_manager:
            ...
            guardrail_result = guardrail_manager.check_user_input(
            ...
            if guardrail_result.violations:
                violation_details = [
                    f"{v.rule_name}: {v.message}" for v in guardrail_result.violations
                ]
                ...
                return {
                    **state,
                    ...
                    "guardrail_violations": [
                        v.__dict__ for v in guardrail_result.violations
                    ],
                }
...
def router_conditional_edge(state):
    ...
    target = (
        router_decision.get("target", "general")
...
def create_agentic_rag_workflow(db_session, config=None):
    ...
    color_logger.info(
        ...
        math_tools_count=len(
            [t for t in tool_names if t in ["add", "subtract", "multiply", "divide"]]
        ),
        router_enabled=True,
        intelligent_routing=True,
    )
    ...
    workflow.add_conditional_edges(
        "router", router_conditional_edge, {"rag_query": "rag_query", "agent": "agent"}
    )
    ...
    color_logger.workflow_complete(
        ...
        persona_enabled=workflow_config.persona_enabled if workflow_config else False,
        persona_name=(
            workflow_config.get_persona_name()
            if workflow_config and workflow_config.persona_enabled
            else None
        ),
