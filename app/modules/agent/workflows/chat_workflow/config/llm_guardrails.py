...
class LLMGuardrailConfig:
	...
	def __post_init__(self):
		...
		if self.monitored_categories is None:
			self.monitored_categories = ['profanity', 'harassment', 'hate_speech', 'violence', 'adult_content', 'privacy_violation', 'spam', 'misinformation', 'brand_safety', 'off_topic', 'security_threat']
	...
	def for_enterview_production(cls) -> 'LLMGuardrailConfig':
		...
		return cls(mode=GuardrailMode.LLM_ONLY, strictness=GuardrailStrictness.STANDARD, model_name='gemini-2.0-flash-lite', temperature=0.1, enable_input_guardrails=True, enable_output_guardrails=True, max_retry_attempts=2, enable_fallback=True, fallback_action='allow_with_warning', min_confidence_threshold=0.7, critical_confidence_threshold=0.9, brand_safety_enabled=True, enterview_specific_rules=True, career_context_awareness=True)

	@classmethod
	def for_development(cls) -> 'LLMGuardrailConfig':
		"""Create relaxed config for development"""
		return cls(mode=GuardrailMode.LLM_ONLY, strictness=GuardrailStrictness.RELAXED, model_name='gemini-2.0-flash-lite', temperature=0.2, enable_input_guardrails=True, enable_output_guardrails=True, max_retry_attempts=1, enable_fallback=True, fallback_action='allow_with_warning', min_confidence_threshold=0.5, critical_confidence_threshold=0.8, brand_safety_enabled=False, enterview_specific_rules=False, career_context_awareness=False)

	@classmethod
	def for_high_security(cls) -> 'LLMGuardrailConfig':
		"""Create high security config"""
		return cls(mode=GuardrailMode.HYBRID, strictness=GuardrailStrictness.STRICT, model_name='gemini-2.0-flash-lite', temperature=0.05, enable_input_guardrails=True, enable_output_guardrails=True, max_retry_attempts=3, enable_fallback=True, fallback_action='block', min_confidence_threshold=0.8, critical_confidence_threshold=0.95, brand_safety_enabled=True, enterview_specific_rules=True, career_context_awareness=True)


# Default configurations for different environments
DEFAULT_CONFIGS = {'production': LLMGuardrailConfig.for_enterview_production(), 'development': LLMGuardrailConfig.for_development(), 'testing': LLMGuardrailConfig.for_development(), 'staging': LLMGuardrailConfig.for_enterview_production(), 'high_security': LLMGuardrailConfig.for_high_security()}
	...
	class MockGuardrailsManager:
		...
		async def validate_user_input(self, user_input: str, context: dict = None):
			...
			try:
				...
				return {'is_safe': result.passed, 'summary': f'Input validation: {"passed" if result.passed else "failed"}', 'violations': [v.message for v in result.violations], 'overall_severity': result.violations[0].severity.value if result.violations else 'low'}
			except Exception as e:
				return {'is_safe': True, 'summary': f'Input validation error: {str(e)}', 'error': str(e)}
		...
		async def validate_ai_response(self, ai_response: str, context: dict = None):
			...
			try:
				...
				return {'is_safe': result.passed, 'summary': f'Output validation: {"passed" if result.passed else "flagged"}', 'violations': [v.message for v in result.violations], 'overall_severity': result.violations[0].severity.value if result.violations else 'low'}
			except Exception as e:
				return {
		...
		async def validate_tool_usage(self, tool_name: str, tool_args: dict, context: dict = None):
			...
			try:
				...
				return {'is_safe': True, 'summary': f'Tool {tool_name} usage validated', 'tool_name': tool_name}
			except Exception as e:
				return {
