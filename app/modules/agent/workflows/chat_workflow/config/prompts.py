...
class SystemPrompts:
    ...
    def get_prompt_stats(cls) -> Dict[str, Any]:
        ...
        color_logger.performance_metric(
            "prompt_analysis_time", f"{analysis_time:.3f}", "s"
        )
...
class PromptTemplates:
    ...
    def get_rag_enhanced_prompt(base_prompt: str, context: str) -> str:
        ...
        if not base_prompt.strip():
            color_logger.warning(
                "Empty base prompt provided", fallback_action="using_default"
            )
            base_prompt = SystemPrompts.DEFAULT_SYSTEM_PROMPT

        if not context.strip():
            color_logger.warning(
                "Empty context provided", fallback_action="using_base_prompt_only"
            )
            return base_prompt

        enhanced_prompt = SystemPrompts.RAG_ENHANCED_TEMPLATE.format(
            base_prompt=base_prompt.strip(), context=context.strip()
        )

        enhancement_time = time.time() - start_time
        color_logger.performance_metric(
            "prompt_enhancement_time", f"{enhancement_time:.3f}", "s"
        )
    ...
    def get_context_specific_prompt(topic: str) -> str:
        ...
        analysis_time = time.time() - start_time
        color_logger.performance_metric(
            "topic_analysis_time", f"{analysis_time:.3f}", "s"
        )
    ...
    def format_documents(docs: list) -> str:
        ...
        if not docs:
            color_logger.warning(
                "No documents provided for formatting", result="empty_context"
            )
            return ""
        ...
        color_logger.performance_metric(
            "doc_formatting_time", f"{formatting_time:.3f}", "s"
        )
    ...
    def create_error_fallback_prompt(error_context: str) -> str:
        ...
        sanitized_context = (
            error_context[:200] + "..." if len(error_context) > 200 else error_context
        )
        ...
        color_logger.performance_metric(
            "fallback_creation_time", f"{creation_time:.3f}", "s"
        )
    ...
    def get_prompt_variations(
        base_topic: str, user_intent: str = "general"
    ) -> List[str]:
        ...
        color_logger.performance_metric(
            "variation_generation_time", f"{generation_time:.3f}", "s"
        )
...
class ValidationPrompts:
    ...
    def validate_response_content(response: str) -> bool:
        ...
        color_logger.performance_metric(
            "safety_validation_time", f"{validation_time:.3f}", "s"
        )
    ...
    def get_safety_score(response: str) -> Dict[str, Any]:
        ...
        for phrase in high_risk_phrases:
            if phrase in response_lower:
                analysis["violations"].append(
                    {"phrase": phrase, "severity": "high", "score_reduction": 0.5}
                )
        ...
        for phrase in medium_risk_phrases:
            if phrase in response_lower:
                analysis["warnings"].append(
                    {"phrase": phrase, "severity": "medium", "score_reduction": 0.2}
                )
        ...
        for phrase in low_risk_phrases:
            if phrase in response_lower:
                analysis["warnings"].append(
                    {"phrase": phrase, "severity": "low", "score_reduction": 0.1}
                )
        ...
        if analysis["warnings"]:
            analysis["recommendations"].append(
                "Cân nhắc điều chỉnh ngôn từ để giảm tính khuyến khích đầu tư"
            )
        if analysis["overall_score"] < 0.8:
            analysis["recommendations"].append(
                "Thêm cảnh báo rủi ro và khuyến nghị tham khảo chuyên gia"
            )

        analysis_time = time.time() - start_time
        color_logger.performance_metric(
            "safety_analysis_time", f"{analysis_time:.3f}", "s"
        )
    ...
    def get_compliance_report() -> Dict[str, Any]:
        ...
        color_logger.performance_metric(
            "compliance_report_time", f"{report_time:.3f}", "s"
        )
