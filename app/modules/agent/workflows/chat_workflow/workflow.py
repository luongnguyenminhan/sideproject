...
7. Tạo câu hỏi thông minh để thu thập thông tin từ người dùng
...
- Question Composer: Tạo câu hỏi thông minh để thu thập thông tin user

📋 SỬ DỤNG QUESTION COMPOSER:
- Khi cần thu thập thông tin về user (CV, kinh nghiệm, kỹ năng, etc.)
- Khi muốn hiểu rõ hơn về background của người dùng
- Để tạo câu hỏi phỏng vấn hoặc đánh giá năng lực
- Để phân tích độ hoàn thiện profile của user

Cách sử dụng:
1. action="generate": Tạo câu hỏi mới
2. action="analyze": Phân tích profile hiện tại
3. Cung cấp existing_user_data với thông tin đã có
4. Có thể chỉ định focus_areas để tập trung vào lĩnh vực cụ thể
