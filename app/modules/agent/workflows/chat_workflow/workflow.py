...
class RouterDecision(BaseModel):
    ...
    target: Literal["rag_query", "agent"] = Field(
        description="Target node để route query đến"
    )
...
class Workflow:
    ...
    def _build_workflow(self) -> StateGraph:
        ...
        workflow.add_conditional_edges(
            "agent", should_continue_wrapper, {"tools": "tools", END: END}
        )
    ...
    async def _router_node(
        ...
        if self.guardrail_manager:
            try:
                ...
                guardrail_result = self.guardrail_manager.check_user_input(
                    user_query, guardrail_context
                )
        ...
        try:
            ...
            router_chain = router_prompt | self.llm.with_structured_output(
                RouterDecision
            )
            router_result = await router_chain.ainvoke({"input": user_query})

            target = (
                router_result.target if hasattr(router_result, "target") else "general"
            )
            explanation = (
                router_result.explanation
                if hasattr(router_result, "explanation")
    ...
    async def _rag_query_node(
        ...
        logger.workflow_start(
            "RAG Query Node - Dual RAG Retrieval", thread_id=thread_id
        )
        ...
        try:
            ...
            try:
                ...
                conv_result = await rag_agent.answer_query(
                    query=user_query, collection_id=collection_id
                )
            ...
            if self.global_kb_service:
                try:
                    ...
                    global_results = (
                        await self.global_kb_service.search_global_knowledge(
                            user_query, top_k=3
                        )
                    )
...
"""
            ...
            error_response = AIMessage(
                content=f"Xin lỗi, có lỗi trong quá trình tạo phản hồi: {str(e)}"
            )
    ...
    def _route_decision(self, state: AgentState) -> str:
        ...
        target = (
            router_decision.get("target", "agent")
    ...
    async def process_message(
        ...
        try:
            ...
            if conversation_id:
                initial_state["conversation_metadata"][
                    "conversation_id"
                ] = conversation_id
            ...
            return {
                ...
                "metadata": {
                    ...
                    "guardrails_passed": not final_state.get(
...
# Factory functions
def create_workflow(
    db_session: Session, config: Optional[WorkflowConfig] = None
) -> Workflow:
