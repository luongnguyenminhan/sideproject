...
class AnalysisWorkflow(BaseWorkflow):
	...
	def __init__(self, config, db: Session = Depends(get_db)):
	...
	async def execute(self, context: Dict[str, Any], api_key: str = None) -> Dict[str, Any]:
		...
		enhanced_context['processing_steps'] = ['data_understanding', 'pattern_recognition', 'insight_generation', 'recommendation_formulation']

		# Execute through LangGraph service
		result = await self.langgraph_service.execute_workflow(agent_config=self.config, context=enhanced_context, api_key=api_key)
	...
	async def execute_streaming(self, context: Dict[str, Any], api_key: str = None) -> AsyncGenerator[Dict[str, Any], None]:
		...
		if not self.validate_context(context):
			yield {'type': 'error', 'message': 'Invalid context for analysis workflow'}
			return
		...
		async for chunk in self.langgraph_service.execute_streaming_workflow(agent_config=self.config, context=enhanced_context, api_key=api_key):
			...
			if chunk.get('type') == 'content':
				...
				for i, step in enumerate(enhanced_context['processing_steps']):
					if step.replace('_', ' ') in content.lower():
						chunk['processing_step'] = {'current': step, 'step_number': i + 1, 'total_steps': len(enhanced_context['processing_steps'])}
						break
	...
	def validate_context(self, context: Dict[str, Any]) -> bool:
		...
		analytical_keywords = ['analyze', 'analysis', 'insights', 'patterns', 'trends', 'data', 'statistics', 'examine', 'investigate', 'study', 'compare', 'evaluate', 'assess', 'review']
	...
	def get_capabilities(self) -> Dict[str, Any]:
		...
		return {
			'streaming': True,
			'memory': True,
			'tools': ['data_analysis', 'pattern_recognition', 'statistical_analysis', 'web_search', 'memory_retrieval'],
			'features': ['structured_analysis', 'insight_generation', 'trend_identification', 'evidence_based_reasoning', 'actionable_recommendations', 'data_visualization_suggestions'],
