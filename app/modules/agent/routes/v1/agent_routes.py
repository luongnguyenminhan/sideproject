...
from app.modules.agent.repository.conversation_workflow_repo import ConversationWorkflowRepo
...
async def update_system_agent_api_key(
    ...
    agent = agent_repo.update_system_agent_api_key(
        api_key=request.api_key, api_provider=request.api_provider
    )
...
async def validate_system_agent(
    ...
    try:
        ...
        # Use a temporary conversation ID for testing
        test_conversation_id = f"test_{uuid.uuid4()}"

        # If override API key provided, temporarily update agent
        if request.override_api_key:
            agent_repo = SystemAgentRepo(db)
            agent_repo.update_system_agent_api_key(request.override_api_key)
