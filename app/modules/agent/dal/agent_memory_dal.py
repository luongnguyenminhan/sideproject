...
class AgentMemoryDAL(BaseDAL[AgentMemory]):
    ...
    def get_memories_by_agent(self, agent_id: str, memory_type: MemoryType = None, 
    ...
    def get_conversation_memories(self, agent_id: str, conversation_id: str) -> List[AgentMemory]:
        ...
        return self.db.query(self.model).filter(
            ...
            self.model.is_deleted == False
        ).order_by(self.model.create_date.asc()).all()
    ...
    def get_important_memories(self, agent_id: str, min_importance: float = 0.7, 
                             limit: int = 20) -> List[AgentMemory]:
        """Get high importance memories"""
        return self.db.query(self.model).filter(
            self.model.agent_id == agent_id,
            self.model.importance_score >= min_importance,
            self.model.is_deleted == False
        ).order_by(desc(self.model.importance_score)).limit(limit).all()
