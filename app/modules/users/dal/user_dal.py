...
class UserDAL(BaseDAL[User]):
    ...
    def get_user_by_email(self, email: str) -> User:
        ...
        return (
            self.db.query(User)
            .filter(User.email == email and User.is_deleted == 0)
            .first()
        )
    ...
    def get_user_by_id(self, user_id: int) -> User:
        ...
        return (
            self.db.query(User)
            .filter(and_(User.id == user_id, User.is_deleted == 0))
            .first()
        )
    ...
    def get_user_by_username(self, username: str) -> User:
        ...
        return (
            self.db.query(User)
            .filter(and_(User.username == username, User.is_deleted == 0))
            .first()
        )
    ...
    def search_users(self, params: dict) -> Pagination[User]:
        ...
        logger.info(
            f"Found {total_count} users, returning page {page} with {len(users)} items"
        )

        return Pagination(
            items=users, total_count=total_count, page=page, page_size=page_size
        )
